...
class Trainer:
    def __init__(
        self, model, train_loader, val_loader, test_loader, optimizer, scheduler, config
    ):
        self.model = model
    ...
    def train_epoch(self, epoch_num, num_epochs):
        self.model.train()
        train_loss = 0.0
        ...
        for step, batch in enumerate(pbar, start=1):
            ...
            pbar.set_postfix(loss=f"{train_loss / step:.4f}", refresh=False)

        return train_loss / len(self.train_loader)

    def validate(self, epoch_num, num_epochs):
        self.model.eval()
        val_loss = 0.0
        all_preds = []
        all_labels = []
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(pbar, start=1):
                ...
                pbar.set_postfix(loss=f"{val_loss / step:.4f}", refresh=False)
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            epoch_num = epoch + 1
            train_loss = self.train_epoch(epoch_num, num_epochs)
            val_loss, val_metrics = self.validate(epoch_num, num_epochs)
